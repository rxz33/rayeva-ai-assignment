...
@router.get(
    "/history/categories",
    tags=["Module 1: AI Auto-Category & Tag Generator"],
    summary="Retrieve past category generation results",
)
def get_category_history(limit: int = 20, db: Session = Depends(get_db)):
    records = (
        db.query(CategoryResult)
        .order_by(CategoryResult.created_at.desc())
        .limit(limit)
        .all()
    )
